...
const App: React.FC = () => {
  ...
  const [aiInsight, setAiInsight] = useState<string>("系统就绪。请配置 Lighter.xyz API 凭据以开启自动跟单...");
    ...
    <div className="min-h-screen p-4 md:p-6 lg:p-10 flex flex-col gap-6 bg-[#020202]">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tight flex items-center gap-3">
              Lighter API 智能跟单网桥
              <span className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded border border-zinc-700 font-mono tracking-widest">专业模式</span>
            </h1>
            <div className="flex gap-3 mt-1 items-center">
              <span className="text-zinc-500 text-[10px] font-mono uppercase">网络: Arbitrum One</span>
              <span className="w-1 h-1 bg-zinc-700 rounded-full"></span>
              <span className="text-zinc-500 text-[10px] font-mono uppercase">API 状态: 已启用</span>
            </div>
          </div>
        </div>
      ...
      <section className="bg-zinc-900/20 border border-zinc-800/50 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="space-y-1">
          <label className="text-[10px] text-zinc-500 font-bold uppercase ml-1">跟单端私钥 (API Key)</label>
          <input 
            type="password"
            ...
            placeholder="请输入私钥以开启自动交易..."
          />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] text-zinc-500 font-bold uppercase ml-1">Arbitrum 节点地址 (RPC)</label>
          <input 
            type="text"
        ...
        <div className="space-y-1">
          <label className="text-[10px] text-zinc-500 font-bold uppercase ml-1">监控带单地址 (Leader)</label>
          <input 
            type="text"
      ...
      <footer className="flex justify-between items-center text-[10px] text-zinc-600 font-mono">
        ...
        <div className="flex gap-4">
          <span className="hover:text-zinc-400 cursor-pointer italic">由 AI 驱动的反向跟单引擎</span>
          <span className="hover:text-zinc-400 cursor-pointer">更新频率: 45秒</span>
        </div>
      </footer>
    </div>
  );
